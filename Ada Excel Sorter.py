...
wb2 = Workbook()
wb = openpyxl.load_workbook('USDA Major Land Use 1945-2012.xlsx')
ws1 = wb.active
...
## A class that sorts through an excel sheet, finds rows that contain the arguements,
## and then creates a new sheet with the rows that contain the arguements
class CellSorter:
...
## Removes anything that isnt a state in column 3   
...
## Removes any states that contain the letter G  
    def RemoveG(self):
        c_row = (0)
        removed_rows = []
        g = ('g')
        for row in self.ws2.iter_rows(min_col=3,max_col=3,max_row=946,min_row=1):
            for cell in row:
                c_row += 1
                if cell.value is not None:
                    state = list(cell.value)
                    for char in state:
                        if char == g:
                            removed_rows.append(c_row)
        
        for row in reversed(removed_rows):
            self.ws2.delete_rows(row)
...
# A variable that contains the letters associated with the desired columns
        
#filter1 = CellSorter(ws,ws2,mincol,maxcol,arguments) 
# Essential creates the filter, you simply define a worksheet to sort through, 
# define a worksheet to populate, define what columns you want to sort with,
# and finally add any additional arguements you'd like, examples being a year, state, or region
        
#filter1.FilteredSheet(columns,1)                
# A method within the cell sorter class that populates a sheet based on the arguements
# provided by filter1 and the columns made as an arguement. the second arguement
# determines the starting row for the population
